...
- [x] 1.1 Add `importConfig` (or extend an existing config) in `src/config/` with the weight token vocabulary as `as const`: `pluma` and `tanque` (no token = `normal` default).
...
- [x] 1.3 Define shared parser types (`ParsedRoster`, `ParsedPlayer`, `ParseWarning`) co-located with the parser util.
...
- [x] 2.4 Implement `parseRosterText(raw): ParsedRoster` — split lines, match the header, match numbered player lines, tolerate spacing/blank/noise lines, and collect warnings.
...
- [x] 5.2 Implement `importTeam(db, input)` in `src/services/import.service.ts`: parse → guard empty → create match → create squad → upsert players → insert lineup (`team='unassigned'`, `batch_id`) → return `{ matchId, createdCount, updatedCount, warnings }`.
- [x] 5.3 On persistence failure, mark the squad row `failed` with the error message and rethrow.

## 6. Action

- [x] 6.1 `src/actions/import.actions.ts`: `importTeam` server action (`'use server'`, try-catch, `logger.action`) returning a typed `ActionResponse` with the success payload or a user-facing error.

## 7. Tests

- [x] 7.1 Unit tests for `slugify`, duplicate-slug suffixing, and `parseRosterText` (well-formed list, spacing variants, blank/noise lines, missing year, `pluma`/`tanque` weight tokens, unknown tokens, duplicate names → `-1`/`-2`).
- [x] 7.2 Add factories in `tests/factories/` for parsed players / match input as needed.
- [x] 7.3 Service test for `importTeam` with mocked repositories: created vs updated counts, lineup all `unassigned`, no duplicate per match, squad recorded.

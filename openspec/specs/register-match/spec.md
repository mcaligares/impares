...
Register a match and its confirmed squad from a raw plain-text paste (e.g. copied from a WhatsApp group), parsing the match header and player lines, upserting players by slug, creating the match, recording the squad batch, and linking players to the match as an unassigned lineup.
